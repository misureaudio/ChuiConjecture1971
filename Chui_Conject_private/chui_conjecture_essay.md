...
*A survey essay for readers in Mathematical Physics, Functional Analysis, and Numerical Analysis.*
...
$$
\operatorname{Re}\Big\langle \frac{1}{z-e^{i\vartheta_1}}, \frac{1}{z-e^{i\vartheta_2}}\Big\rangle_{A^2_{(g)}} = \kappa_g\, \varphi_{(g)}(\vartheta_2 - \vartheta_1), \qquad
\varphi_{(g)}(t) = \sum_{k\ge 0} c_{(g),k}\cos((k+1)t),\quad c_{(g),k} = \int_0^1 s^k g(1-s)\,ds.
$$
